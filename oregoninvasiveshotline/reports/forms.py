...
from django.db import transactionfrom collections import namedtuple
...
class ReportForm(forms.ModelForm):
    ...
    def clean_point(self):
        ...
        # Oregon bounding box (approximately)
        min_latitude = 42.0
        max_latitude = 46.3
        min_longitude = -124.5
        max_longitude = -116.5
        ...
        if not point:
            # If the point is missing entirely, return a general error message
            raise ValidationError("Please drag the pin to the location where the specimen was found.")

        latitude = point.y
        longitude = point.x

        # Check if the point is outside Oregon's boundaries
        if not (min_latitude <= latitude <= max_latitude and min_longitude <= longitude <= max_longitude):
            raise ValidationError("The location you selected is outside Oregon. Please place the pin within Oregon.")
